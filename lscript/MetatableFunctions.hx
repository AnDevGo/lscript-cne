...
class MetatableFunctions {
    ...
    static function metatableFunc(state:State, funcNum:Int) {
        ...
        // 直接传递数组，而不是使用 RawPointer.addressOf
        final params:Array<Dynamic> = [for(i in 0...nparams) CustomConvert.fromLua(-nparams + i, specialIndex, parentIndex, i == 0)];
        
        if (funcNum == 2) {
            // 使用数组索引访问值
            final objParent = (parentIndex[0] >= 0) ? LScript.currentLua.specialVars[parentIndex[0]] : null;
        ...
        if (returned != null) {
            // 使用数组索引访问值
            CustomConvert.toLua(returned, funcNum < 2 ? specialIndex[0] : -1);
            return 1;
        }
        return 0;
    }
